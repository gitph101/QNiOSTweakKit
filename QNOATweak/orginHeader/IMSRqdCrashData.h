//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class IMSRqdThreadInfo, NSArray, NSException, NSMutableArray, NSMutableDictionary, NSMutableString, NSString;

@interface IMSRqdCrashData : NSObject
{
    long long time;
    NSString *type;
    NSString *error;
    NSString *address;
    NSString *errorStack;
    NSString *thread;
    int crashThread;
    NSString *processName;
    NSString *processUUID;
    NSString *processArch;
    NSString *processAddr;
    NSMutableArray *crashBinInfos;
    NSArray *crashStackShortNames;
    NSArray *allStackShortNames;
    NSException *exp;
    NSMutableString *crashlog;
    NSString *handler;
    NSMutableArray *sessionEvents;
    NSMutableDictionary *userMaps;
    NSMutableArray *allThreadInfos;
    IMSRqdThreadInfo *crashThreadInfo;
    IMSRqdThreadInfo *ocThreadInfo;
    NSArray *ocStackInfos;
    NSMutableString *registerDatas;
    NSString *objcMsgData;
    NSMutableDictionary *sceneData;
    _Bool crashOnceMore;
    NSMutableString *lastExceptionInfo;
    _Bool customizedException;
    IMSRqdCrashData *_currentCustomizedCrashData;
    NSArray *_sdkCrashCallStack;
}

+ (id)expData;
@property(retain, nonatomic) NSArray *sdkCrashCallStack; // @synthesize sdkCrashCallStack=_sdkCrashCallStack;
@property(retain, nonatomic) IMSRqdCrashData *currentCustomizedCrashData; // @synthesize currentCustomizedCrashData=_currentCustomizedCrashData;
@property(nonatomic, getter=isCustomizedException) _Bool customizedException; // @synthesize customizedException;
@property(retain, nonatomic) NSMutableString *lastExceptionInfo; // @synthesize lastExceptionInfo;
@property(nonatomic) _Bool crashOnceMore; // @synthesize crashOnceMore;
@property(retain, nonatomic) NSMutableDictionary *sceneData; // @synthesize sceneData;
@property(retain, nonatomic) NSString *objcMsgData; // @synthesize objcMsgData;
@property(retain, nonatomic) NSMutableString *registerDatas; // @synthesize registerDatas;
@property(retain, nonatomic) IMSRqdThreadInfo *ocThreadInfo; // @synthesize ocThreadInfo;
@property(retain, nonatomic) IMSRqdThreadInfo *crashThreadInfo; // @synthesize crashThreadInfo;
@property(retain, nonatomic) NSMutableArray *allThreadInfos; // @synthesize allThreadInfos;
@property(retain, nonatomic) NSMutableDictionary *userMaps; // @synthesize userMaps;
@property(retain, nonatomic) NSMutableArray *sessionEvents; // @synthesize sessionEvents;
@property(retain, nonatomic) NSMutableString *crashlog; // @synthesize crashlog;
@property(retain, nonatomic) NSMutableArray *crashBinInfos; // @synthesize crashBinInfos;
@property(retain, nonatomic) NSString *processAddr; // @synthesize processAddr;
@property(retain, nonatomic) NSString *processUUID; // @synthesize processUUID;
@property(retain, nonatomic) NSString *processArch; // @synthesize processArch;
@property(retain, nonatomic) NSString *processName; // @synthesize processName;
@property(retain, nonatomic) NSString *handler; // @synthesize handler;
@property(retain, nonatomic) NSException *exp; // @synthesize exp;
@property(retain, nonatomic) NSArray *allStackShortNames; // @synthesize allStackShortNames;
@property(retain, nonatomic) NSArray *crashStackShortNames; // @synthesize crashStackShortNames;
@property(nonatomic) int crashThread; // @synthesize crashThread;
@property(retain, nonatomic) NSString *errorStack; // @synthesize errorStack;
@property(retain, nonatomic) NSString *address; // @synthesize address;
@property(retain, nonatomic) NSString *thread; // @synthesize thread;
@property(retain, nonatomic) NSString *error; // @synthesize error;
@property(retain, nonatomic) NSString *type; // @synthesize type;
@property(nonatomic) long long time; // @synthesize time;
- (void).cxx_destruct;
@property(readonly, copy) NSString *crashStack;
- (void)addLastExceptionInfo:(id)arg1;
- (_Bool)isUnityException;
- (void)addSdkSceneValue:(id)arg1 forKey:(id)arg2;
- (void)setSdkScene:(unsigned long long)arg1;
- (void)addSceneValue:(id)arg1 forKey:(id)arg2;
- (unsigned long long)currentScene;
- (void)setScene:(unsigned long long)arg1;
- (id)allSceneValues;
- (id)sceneValueForKey:(id)arg1;
- (void)removeAllSceneValues;
- (void)removeSceneValueForKey:(id)arg1;
- (void)addUserMapData:(id)arg1 val:(id)arg2;
- (void)appendSessionEvent:(id)arg1;
- (_Bool)needCheckFirstLineOcMsg;
- (id)init;

@end

